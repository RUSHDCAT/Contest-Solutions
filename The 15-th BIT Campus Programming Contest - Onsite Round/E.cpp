...
void del(int x){
	if(x!=-1)
	ans.erase(ans.find( max( LCP(pre[x],x), LCP(x,nex[l]) ) + 1 ));
}
void add(int x){
	if(x!=-1)
	ans.insert(max( LCP(pre[x],x), LCP(x,nex[l]) ) + 1);
}
void add(int l,int m,int r){
	del(l); del(r);
	if(l>=0)nex[l]=m,pre[m]=l;
	if(r>=0)pre[r]=m,nex[m]=r;
	add(l); add(r); add(m);
}
...
void buildSTK(){
	...
	for(int i=n-1;i>=0;i--){
		pair<int,int> now = make_pair(rk[i], i);
		if(i==n-1) {
			ans.insert(1); continue;
		}
		auto r = pos.upper_bound(now);
		auto l = prev(r);
		add((*l).second,i,(*r).second)
	}
}
int main() {
	scanf("%d%s",&n,s);
	for(int i=0;i<n;i++){
		if(n-1-i>i) swap(s[i],s[n-1-i]);
	}
	buildSA(1000); buildH(); RmqInit(); buildSTK();
}